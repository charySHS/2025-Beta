...
public class LookupTable
{
    ...
    {
        assert Nodes.size() > 1 : "LookupTable requires two or more values to lerp between";

        var numNodes = Nodes.size();

        // -- Clamp low
        var low = Nodes.get(0);
        if (input <= low.x) { return low.y; }

        // -- Clamp high
        var high = Nodes.get(numNodes - 1);
        if (input >= high.x) { return high.y; }
        ...
        // -- Early out if outputs are same to avoid div by 0
    ...
    static public class Normalized extends LookupTable
    {
        ...
        public LookupTable AddValue(double input, double output)
        {
            assert input > 0 && input < 1 : "Normalized lookup table must have input in range of 0 to 1";
